...
namespace UseCase.UserServices
{
    public class AuthentificationOptions(IConfiguration configuration)
    {
        ...
        public string AUDIENCE { get; } = configuration["JWTBearer:AUDIENCE"] ?? throw new Exception("\"AUDIENCE\" not found in configuration");
        public string KEY { get; } = configuration["JWTBearer:KEY"] ?? throw new Exception("\"KEY\" not found in configuration");
